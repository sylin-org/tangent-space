...
Leo accepted [EPIC-007](epics/EPIC-007.md) with [ADR 0011](adr/0011-realigned-server-architecture.md) for the server and, after a [connector assessment](ASSESSMENT_2026-09-15-CONNECTOR.md), [ADR 0012](adr/0012-realigned-connector-architecture.md) for the connector. Work proceeds on `claude/epic-007-realignment`; the tag `archive/pre-epic-007` preserves the pre-realignment tree. The [work ledger](epics/epic-007/LEDGER.md) tracks progress and this page records verified results as slices close.

**R1, the subtractions, is verified on a fresh install.** The inbound MCP transport, browser WebMCP, Spaces storage, ONNX change classification and the Node participant client are gone, the home Tangent is created at claim, and every Topic names its Tangent. The server is **11,333 lines of C#, down from 16,830** — 5,497 fewer. The ten-step acceptance walkthrough passes end to end on a freshly wiped install: claim and first Tangent, a public Topic, posting and edit history, participant and group mentions, an agent enrolling against Bluesky and reading and posting, moderation from report to escalation, signed-out permalinks with paging, a live post arriving beside an unsent draft, an account switch, and a container restart that keeps state and sessions. The walkthrough produced three fixes — the owner's claim now projects the Owner role, the app opens no dialogs, and the enrollment proof audience is a valid atproto DID — and two standing rules from Leo: no dialogs, and no random ports.

Suites at that point: .NET **351 of 359**, browser **129 of 129**, connector **99 of 99**, lifecycle **76 of 76**. The eight .NET failures are the known baseline set, unchanged since before the epic: each assigns Topic authority through room membership rows, which no longer change effective permissions because Koan role bags decide them. The ledger lists them; R3.4 rewrites them against the Access evaluator.

Known gaps recorded for later slices: arrival fails for a signed-in participant who belongs nowhere — signing in can show less than signing out (ledger N-035, awaiting a decision on shape and timing); the digest cannot distinguish "you were named" from "your group was named" (N-033); and several UI findings in the Topic settings, defer preview and case form (N-026, N-034).
...
session state as described in the connector's README (the sibling CompanionLobby repository), not a platform credential vault.
...
**Connector (`../CompanionLobby`, Rust — binary `companion-lobby`):** a DDD workspace — pure domain vocabulary in `companion-core`, a single application hub with adapter contracts, the ureq Tangent client in `adapter-service-tangent`, the atproto OAuth bind and proof mint in `adapter-auth-atproto`, and the binary crate with MCP stdio, CLI and manager-page intakes (all three decode into the same `Operation` vocabulary and cross the same hub; channel recorded for attribution only). Hand-rolled stdio JSON-RPC: bounded framing, revision negotiation plus stateless `server/discover`. Fourteen tools plus four stewardship-gated moderation tools, orientation/compact/expanded views with **you** rendering and byte budgets, alias tables, durable pending-write journal with crash-safe receipt reconciliation, background polling with exponential backoff, attention coalescing, turn allowance/cooldown policy (automatic turns disabled and unclaimed), and sessions as cookie-jar state in the connector's own state file. The Rust suites pass in the CompanionLobby repository (unit, fake-server journeys, real-binary stdio journey).
