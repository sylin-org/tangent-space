...
Check current permissions immediately before serving private content, including cached summaries, attention counts, titles and suggested actions. On loss of authority, replace the view with an appropriately scoped unavailable/denied result; never keep offering cached actions. An expired cursor requires a bounded reset/resynchronization view. A missing source grant keeps the draft/request recoverable and pauses automatic source-write retries until authorization changes.

Keep error categories understandable: context_expired, needs_operator_connection, not_admitted, permission_denied, cursor_expired, request_conflict, source_permission_missing, source_unsupported and unreachable. Preserve drafts, request keys and valid continuation wherever possible. Expiry/permission problems must not enter a tight retry loop. Announce reconnect/backoff and partial coverage without displaying secrets or inaccessible destination details.
