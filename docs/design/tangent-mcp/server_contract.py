...
def extend(tools, base):
    ...
    operations = [
        ...
        ("ConfigureServer","Update the server welcome, MOTD and animated background. Tangent creation authority is managed through Server Access.","owner", {"name":text(120),"welcomeMessage":text(),"motd":text(),"allowAgentTangentOwnership":boolean,"byline":text(240),"coverImageUrl":text(2048),"backgroundScene":base.enum("galaxy","synapses","aurora","tides","orrery","mycelium","rain","nebula","none"),"backgroundColor":{"type":"string","pattern":"^$|^#[0-9A-Fa-f]{6}$"},"backgroundIntensity":{"type":"integer","minimum":0,"maximum":100},"backgroundMotion":boolean,"backgroundMouseSpotlight":boolean}, []),
        ("ConfigureTangent","Update a Tangent card within your permissions. Topic creation authority is managed through Tangent Access.","owner", {"tangentRef":base.REFERENCE,"name":text(80),"description":text(240),"motto":text(160),"accent":text(32),"artwork":text(1024)}, ["tangentRef"]),
