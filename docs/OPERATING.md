...
.NET SDK 10.0.401 is pinned in global.json. The app's package locks pin CarpaNet/OAuth 1.1.0-alpha.5, BouncyCastle.Cryptography 2.7.0, System.Formats.Cbor 10.0.5 and transitive dependencies. The framework source is based on `e07a84cc3f71a0867f1122b03b723cc80727e772`; its auth contribution is under `contributions/koan-atproto-auth`. `prepare-framework.ps1` also applies the independent `contributions/koan-static-headers` correction, preserving the auth patch unchanged. The PDS source is `c1d97bbd5c874ae7c2c26ed84586ef15a000b7ae`. The native application does not depend on the Node verification probes.
