...
**For the local MCP connector implementation, use [IMPLEMENT_LOCAL_MCP](IMPLEMENT_LOCAL_MCP.md).** This older general prompt describes the original Windows handoff and historical runtime state. The accepted v1 architecture is now the [server experience API with a local connector](../design/experience-api/README.md).
...
The original handoff captured modified/untracked work after b2abeff; that prototype was subsequently published in commit 81ec80f. Inspect and preserve actual local modifications. The handoff kit contains no credentials or runtime backup. Use Build.bat/Launch.bat for the app; Backup.bat/Restore.bat copy its host-mounted state. Wipe.bat is for a deliberately requested clean start, not normal setup.
