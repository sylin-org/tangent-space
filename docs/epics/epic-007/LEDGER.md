...
| Current task | R2.1 — apply the glossary (`doing`), stage 3 of four |
| Next action | R2.1 stage 3: **`Message` to `Post`** — 158 occurrences in 39 files, and the entity is persisted, so N-044 applies again. Expect an N-045 collision: `References` already has a `Post`-shaped member. The boundary stays where stages 1 and 2 put it: C# types, their family and C# prose only; `wwwroot/`, routes, reason codes and serialized property names are wire and belong to R2.4. Then stage 4 (`Room` to `Topic`), the largest. Frozen test files are only kept compiling |
| Last checkpoint | 2026-09-16 · S-004 · R2.1 stage 2 done and committed: `TangentCommunity` is `Tangent`. .NET 87/87, browser 7/7, greenfield 1,444 → 1,385, build warnings unchanged at 58 |
