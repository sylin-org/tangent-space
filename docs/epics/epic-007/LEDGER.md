...
| Current task | R2.3 — move folders into the modules of ARCHITECTURE. R2.1 stays `waiting` on Leo for the relaunch |
| Next action | **R2.3 — move folders into the modules of [ARCHITECTURE](../../ARCHITECTURE.md#modules)**, including `Mcp/Authentication` into Identity. This is the task that retires the `TangentSpace.Rooms` namespace, which is 82 of the remaining findings, and it is the last rename-shaped task before R2.4 takes the wire. Still owed to Leo, neither blocking: the relaunch timing (N-044), the `probes/` question (N-047), and whether R2.2's split should have been five services rather than five files (N-050) |
| Last checkpoint | 2026-09-16 · S-004 · R2.2 done and committed: `CompanionGovernance` is `ParticipantGovernance` across six files, one responsibility each. Greenfield 858 → 819; .NET 87/87, browser 7/7 |
