...
| Current task | R2.1 — apply the glossary (`doing`), stage 4 of four |
| Next action | R2.1 stage 4: **`Room` to `Topic`**, the largest — 684 occurrences in 87 files, plus `Room.Topic` (the description text) becoming `Topic.Description`. `References.IsTopicKey` calls `Room.CheckKey`, which is N-045's collision waiting on the line below the one stage 2 fixed. Run N-046's literal audit before committing: `room` appears in stored keys (`RoomKey`), routes (`/api/rooms`) and reason codes. `probes/` compiles against the server and uses `RoomKey` (N-047). Then verify: four suites, greenfield and a relaunch |
| Last checkpoint | 2026-09-16 · S-004 · R2.1 stage 3 done and committed: `Message` is `Post`. .NET 87/87, browser 7/7, greenfield 1,385 → 1,366; server, tests and all five probes build |
...
| Waiting on | Leo: push authorization (optional) — eleven commits exist only on this machine. N-035 is settled — D11 and D12, [ADR 0013](../../adr/0013-access-contract-and-role-model.md) |
