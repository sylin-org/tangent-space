...
| Current task | R2.1 — apply the glossary (`todo`) |
| Next action | Start R2.1: **narrow the greenfield check's Spaces rule to `SourceDecision` first** (N-038), then apply the glossary — `TangentSite` to `Space`, `TangentCommunity` to `Tangent`, `Room` to `Topic`, `Message` to `Post`. Frozen test files are only kept compiling |
