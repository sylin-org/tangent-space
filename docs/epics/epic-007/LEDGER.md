...
| Current task | R2.1 — apply the glossary (`waiting` on Leo for the relaunch). R2.2 can start without it |
| Next action | **R2.2 — split `CompanionGovernance` by responsibility** (membership, admission, watches, classification). It is the largest C# file left at 59 retired-vocabulary findings, and it needs no wipe, so it proceeds while the relaunch waits. Two things are owed to Leo, neither blocking: the relaunch timing (N-044) and the `probes/` question (N-047). R2.4 now has an itemized inbox of everything the rename stages deliberately left (N-049) |
| Last checkpoint | 2026-09-16 · S-004 · **R2.1's four rename stages are done and committed.** Space, Tangent, Topic and Post are the code's words. Greenfield 1,496 → 858; .NET 87/87, browser 7/7, connector 52/52, lifecycle 30/30 |
