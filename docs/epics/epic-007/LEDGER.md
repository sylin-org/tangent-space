...
| Current task | R1.5 — move the proof audience and DID-key resolution into Identity; remove Spaces storage (`todo`, next) |
| Next action | Start R1.5: set it to `doing` and follow the steps under [In flight](#in-flight) |
| Last checkpoint | 2026-09-15 · S-002 · R1.4 complete; committed as "refactor: every Topic names its Tangent" (R1.3 is `c3a4b9b`) |
...
**R1.5 — move the proof audience and DID-key resolution into Identity; remove Spaces storage** (next; not started)

- [ ] Move the enrollment proof audience (`Tangent:Spaces:ManagingApp`, read by `ServiceProofAuthentication`) and DID-key resolution (`SpacesVerifier`, `ResolvedAuthorKey`) into Identity; per D9 the audience defaults from `Tangent:Site:PublicOrigin`, with an override
- [ ] Inventory Spaces storage: services, notifications, readiness, controllers (`/provision`, connections), `SpaceCarVerifier`, `ConversationSync`, `WriteIntent`, Spaces branches in conversation writes and acceptance, `RoomSpaceState`/`SpaceUri`/`CompleteSpace`, BouncyCastle and CBOR packages, the fixture-network launch path and configuration, probes and remaining scripts (`prove-arrival-suspension`, `prove-pending-access` and others), the `rooms.js` Spaces status UI, the discovery document's `sourceWriteConsent`
...
- R1.4: every Topic names its Tangent; `POST /api/rooms` and five fixture-network scripts removed; the digest reads facets only. .NET 455/467 with only the 12 known failures; browser 139/139; greenfield 2,473.
- Next: R1.5.
