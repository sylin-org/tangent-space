...
| R1.15 | The test audit (Leo, 15 September). Classify every test rule / contract / mechanism against the guardrail; delete the mechanism tier whose subject is already gone; mark the rest **frozen** — owned by the task that replaces their subject, never carried through R2's renames; add the rule tests the R1 walkthrough shows were missing (N-040) | todo | Every test file is classified; no frozen test is renamed by R2; the three missing rule tests exist | |
...
- **N-040** (R1.15) The test inventory behind Leo's judgement, counted on 15 September. **656 tests**: .NET 357, browser 129, connector 94 (plus about 35 unit tests inside its source), lifecycle 76.
  ...
  The concentration is the point. Tests attached to components this epic has already accepted replacing: `TopicPermissionEvaluatorTests` 15 and `AccessMapTests` 5 (R3.4 and D11 replace both), `RoomRulesTests` 13 (R2.1 renames `Room`; the file alone holds 102 greenfield findings), `ParticipantLabelWindowTests` and `WindowPlannerTests` 13 (R5.1 leaves one window), and in the browser `window-contract` 20, `activity-transport` 21 and `room-recovery` 13 (R3.5 and R5 replace their subjects). The eight [known failures](#known-baseline-failures) live in exactly two files, both of which R3.4 and R4.1 rewrite. The connector's fake is **1,471 lines**, over half the size of the five journey files it serves, and C7 already records that it has drifted from the real server.
