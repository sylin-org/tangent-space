...
| Current task | R2.4 — rename the wire fields, in server, browser and connector together. R2.1 stays `waiting` on Leo for the relaunch |
| Next action | **R2.4 — the wire**, which is where four rename stages and two module moves have been depositing work: `RoomKey`/`roomKey` (303), `channel*` (168), all of `wwwroot/` (~330), `ActivityKind.RoomChanged`, `TopicListing.Rooms`, the `"room-not-found"` reason code, `CompanionRoleResult.Community`, `OfMessageId`/`ofMessageId`, the `messages` and `api/rooms` routes, and `Room.Topic`'s DTO spelling. N-049 itemizes them with owners. This is the first task that must change the browser, and R1.15 deleted the suites that would catch a mistake there — so it needs a plan for how a wire change is proved, not just made |
| Last checkpoint | 2026-09-16 · S-004 · R2.3 done and committed: 181 files into ten `Tangent.<Module>` namespaces, `Mcp/` gone. Greenfield 720; .NET 87/87, browser 7/7, five probes build |
...
- Next: R2.2 splits `CompanionGovernance`, the largest C# file left, and needs no wipe.
