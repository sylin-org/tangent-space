...
internal static class Fixture
{
    ...
    internal static string Id(string topic, long sequence) => "mongo-" + topic + "-" + sequence.ToString("D10");
    internal static string Text(long sequence) => sequence % 97 == 0 ? new string('x', 3900) : "Synthetic @member #scale " + sequence + " — café 日本語";
    internal static Post Make(string topic, long sequence) => new()
    {
        Id = Id(topic, sequence), RoomKey = topic, Sequence = sequence,
    ...
    internal static void Validate(IReadOnlyList<Post> rows, string topic, long edge, bool descending)
    {
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            ...
            if (rows[i].Id != Id(topic, sequence) || rows[i].Sequence != sequence || rows[i].RoomKey != topic
