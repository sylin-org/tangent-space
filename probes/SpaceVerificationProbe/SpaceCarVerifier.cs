...
namespace SpaceVerificationProbe;

// Experimental protocol composition for the pinned alpha, not a crypto primitive
// implementation. The caller supplies an independently resolved author key.
public sealed record ResolvedAuthorKey(string AuthorDid, string Curve, byte[] PublicKey);
...
public static class SpaceCarVerifier
{
        ...
        ResolvedAuthorKey key, bool expectValues = true)
    {
        ...
        Require(VerifySignature(key, context, commit.Signature), "Commit signature mismatch");
    ...
    private static bool VerifySignature(ResolvedAuthorKey key, byte[] context, byte[] signature)
    {
        var curveName = key.Curve switch { "secp256k1" => "secp256k1", "P-256" => "secp256r1", _ => throw new InvalidDataException("Unsupported author key curve") };
        var curve = CustomNamedCurves.GetByName(curveName);
        var domain = new ECDomainParameters(curve.Curve, curve.G, curve.N, curve.H);
        var point = curve.Curve.DecodePoint(key.PublicKey);
        var publicKey = new ECPublicKeyParameters(point, domain);
        var r = new BcInteger(1, signature[..32]);
        var s = new BcInteger(1, signature[32..]);
        if (r.SignValue <= 0 || r.CompareTo(curve.N) >= 0 || s.SignValue <= 0 || s.CompareTo(curve.N.ShiftRight(1)) > 0) return false;
        var verifier = new ECDsaSigner();
        verifier.Init(false, publicKey);
        return verifier.VerifySignature(SHA256.HashData(context), r, s);
    }
    ...
    {
        Require(car.Length > 0 && car.Length <= MaxCarBytes, "CAR size outside probe bound");
    ...
    {
        Require(depth <= 64, "CBOR nesting exceeds probe bound");
