...
/// <summary>A current document-controlled AT signing key. JWT claims and algorithm checks belong to the caller.</summary>
public sealed class ResolvedAuthorKey
{
    ...
    public static ResolvedAuthorKey FromDidDocument(DidDocument document, string expectedAuthor)
    {
        ArgumentNullException.ThrowIfNull(document);
        if (document.Id != expectedAuthor || string.IsNullOrWhiteSpace(expectedAuthor))
            throw new InvalidDataException("Signing-key document belongs to a different author.");
        var candidates = document.VerificationMethod?.Where(method => method is not null && (method.Id == "#atproto" || method.Id == expectedAuthor + "#atproto")).ToArray() ?? [];
        if (candidates.Length != 1 || candidates[0].Controller != expectedAuthor)
        ...
        {
            // The pinned official DID resolver supports these legacy documents:
            // the multibase contains the raw EC point, not a multicodec envelope.
            if (!(encoded.Length == 33 && encoded[0] is 2 or 3) && !(encoded.Length == 65 && encoded[0] == 4))
                throw new InvalidDataException("Expected a compressed or uncompressed legacy AT signing key.");
            return new(expectedAuthor, method.Type == "EcdsaSecp256k1VerificationKey2019" ? "secp256k1" : "P-256", encoded);
        }
        if (method.Type != "Multikey" || encoded.Length != 35)
            throw new InvalidDataException("Expected a supported AT Multikey envelope.");
        var curve = (encoded[0], encoded[1]) switch
        {
            (0xe7, 0x01) => "secp256k1",
            (0x80, 0x24) => "P-256",
            _ => throw new InvalidDataException("Unsupported AT signing-key multicodec.")
        };
        if (encoded[2] is not (2 or 3)) throw new InvalidDataException("Expected a compressed AT signing key.");
        return new(expectedAuthor, curve, encoded[2..]);
    }
    ...
    // CarpaNet's base58 helpers are private. This bounded encoding conversion
    // handles only bounded AT key encodings; curve math stays in BouncyCastle.
    private static byte[] DecodeBase58(string? value)
    {
        ...
        if (value is null || value.Length is < 2 or > 128 || value[0] != 'z' || value[1] == '1')
            throw new InvalidDataException("Expected a bounded base58btc AT Multikey.");
        var number = BigInteger.Zero;
        foreach (var character in value.AsSpan(1))
        {
            var digit = alphabet.IndexOf(character);
            if (digit < 0) throw new InvalidDataException("Invalid base58btc AT Multikey.");
            number = number * 58 + digit;
        }
        var bytes = number.ToByteArray(isUnsigned: true, isBigEndian: true);
        if (bytes.Length is < 33 or > 65) throw new InvalidDataException("AT signing key exceeds its supported encoding bound.");
        return bytes;
    }
}
