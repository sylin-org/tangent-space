...
public sealed partial class ExperienceService
{
    ...
    public async Task<ExperienceResponse> Mentionables(ClaimsPrincipal principal, string topicKey, string? prefix, CancellationToken ct)
    {
        ...
        Topic? stored;
        Community.Tangent? tangentRow;
        using (EntityContext.NoCache())
        {
            stored = await Topic.Get(topicKey, ct);
            tangentRow = stored is null ? null : await Community.Tangent.Get(stored.TangentKey, ct);
        }
        ...
        using (EntityContext.NoCache())
        {
            if ((await Space.Get(TangentConstants.SpaceId, ct)) is { } siteRow)
