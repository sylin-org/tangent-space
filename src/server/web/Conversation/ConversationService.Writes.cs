...
{
    public Task<PendingMessagePage> PendingMessages(string participantId, string roomKey, CancellationToken ct)
        => governance.WithCurrentPolicy(participantId, roomKey, async (policy, token) =>
        {
            ...
            var pending = await WriteIntent.Query(i => i.AuthorParticipantId == participantId && i.RoomKey == roomKey && i.State == "pending",
    ...
    public Task<WriteIntent> Post(string participantId, string roomKey, PostMessage input, CancellationToken ct)
        => Post(participantId, roomKey, input, ct, background: false);

    private async Task<WriteIntent> Post(string participantId, string roomKey, PostMessage input, CancellationToken ct, bool background)
    {
        ...
        try
        {
            ...
            using (var scope = EntityContext.NoCache())
            {
                ...
                if (local.SpaceState == RoomSpaceState.Local) return await UpsertLocalPost(participantId, roomKey, input, facets, ct);
            }
            // Source writes are atproto-scoped: the Spaces pipeline writes to the acting
            // participant's atproto repository and crosses to participants here.
            var authorDid = await directory.AtprotoDidOf(participantId, ct)
                ?? throw new UnauthorizedAccessException("Posting to a Spaces Topic requires an atproto identity.");
            ...
            var intent = await governance.WithCurrentPolicy(participantId, roomKey, async (policy, token) =>
            {
                ...
                var created = new WriteIntent { Id = id, AuthorParticipantId = participantId, RoomKey = roomKey,
            ...
            try
            {
                ...
                await governance.WithCurrentPolicy(participantId, roomKey, async (_, token) =>
                {
                    intent.SourceUri = intent.SpaceUri + "/" + authorDid + "/" + SpacesOptions.Collection + "/" + intent.RecordKey;
    ...
    private async Task<WriteIntent> UpsertLocalPost(string participantId, string roomKey, PostMessage input, IReadOnlyList<PostFacet>? facets, CancellationToken ct)
    {
        ...
        await governance.WithCurrentPolicy(participantId, roomKey, async (policy, token) =>
        {
            ...
            if (existing is { } found)
            {
                ...
                result = Carried(participantId, roomKey, input, facets, uri, cid, found.Content.CreatedAt);
                return true;
            }
            if (input.ReplyTo is { } reply)
            {
            ...
            result = Carried(participantId, roomKey, input, facets, uri, cid, content.CreatedAt);
            return true;
        }, ct);
    ...
    private static WriteIntent Carried(string participantId, string roomKey, PostMessage input, IReadOnlyList<PostFacet>? facets, string uri, string cid, DateTimeOffset createdAt)
        => new()
        {
            AuthorParticipantId = participantId, RoomKey = roomKey, OperationId = input.OperationId,
