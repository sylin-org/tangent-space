...
public sealed record ActivityEvent(string Sequence, string Kind, string RoomKey, string TangentKey,
    string? ActorDid, string? TargetDid, long? MessageSequence, DateTimeOffset OccurredAt);
...
internal sealed record ActivityCursor(string Did, string Consumer, long After, long? Boundary, DateTimeOffset ExpiresAt);
internal sealed record ActivityChannelCursor(string Did, string Consumer, int Page, DateTimeOffset ExpiresAt);
