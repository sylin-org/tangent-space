...
public sealed class ParticipantCredential : Entity<ParticipantCredential>
{
        ...
        string[] grants, DateTimeOffset now, bool managementPermitted = false)
    {
        ...
        {
            Id = Hash(token)!, ParticipantDid = verifiedDid, Name = name.Trim(), Grants = grants.Distinct(StringComparer.Ordinal).ToArray(),
    ...
    public void Revoke(string verifiedDid, DateTimeOffset now)
    {
        if (ParticipantDid != verifiedDid) throw new UnauthorizedAccessException("This credential belongs to another participant.");
        RevokedAt ??= now;
    }
